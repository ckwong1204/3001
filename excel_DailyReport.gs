function calc_DailyReport_test ( row, date ){ 
  //get last row  
  init_DailyReport(4, '171124');
  calc_DailyReport(4, '171124');
}
...
function calc_DailyReport ( row, date ){
  try{
    ...
    var date        = valueList[1]; // B: 第一交易日 yymmdd
    ...
    if (date  <=  today_yymmdd) { // if current Date >= 第一交易日
      var data_1st = get_hsio_json(date, contracts);
      if(get_hsio_json){
        sheet.getRange('H'+row).setValue( data_1st[ date + Curr_C_str ].OQP_CLOSE ); // H: 交易日 即月 Call  date + CommonData.Month[contract_curr.month] + "-" + contract_curr.year + "-" + strike_curr + "-" + "C"
        sheet.getRange('I'+row).setValue( data_1st[ date + Curr_P_str ].OQP_CLOSE ); // I: 交易日 即月 Put   date + CommonData.Month[contract_curr.month] + "-" + contract_curr.year + "-" + strike_curr + "-" + "P"
        sheet.getRange('J'+row).setValue( data_1st[ date + Next_C_str ].OQP_CLOSE ); // J: 交易日 下月 Call  date + CommonData.Month[contract_next.month] + "-" + contract_next.year + "-" + strike_next + "-" + "C"
        sheet.getRange('K'+row).setValue( data_1st[ date + Next_P_str ].OQP_CLOSE ); // K: 交易日 下月 Put   date + CommonData.Month[contract_next.month] + "-" + contract_next.year + "-" + strike_next + "-" + "P"      
